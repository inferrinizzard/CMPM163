Shader "Custom/phongTexture"
{
	...
	SubShader{
		...
		Pass{
			...
			uniform fixed4 _Color, _SpecularColor;
			...
			fixed4 frag(v2f i):COLOR{
				...
				return float4((ambient + diffuse) * tex2D(_Texture, i.uv) + specular, 1.0);
			}
		...
		Pass {
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				float3 diffuse = _LightColor0.rgb * _Color.rgb * max(0.0, dot(normal, lightDirection)); //Diffuse component
				float3 specular = dot(i.normal, lightDirection) < 0.0 ? float3(0.0, 0.0, 0.0) : 
							_LightColor0.rgb * _SpecColor.rgb * pow(max(0.0, dot(reflect(-lightDirection, normal), view)), _Shininess);
